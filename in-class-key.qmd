...
for( i in 2:nrow(df) ) { 
  ...
  df$py[i] <- df$py[(i-1)]*(1-(mxy + myz) ) + df$px[(i-1)]*mxy + df$pz[(i-1)]*mxz
  df$pz[i] <- df$pz[(i-1)]*(1-(myz + mxz) ) + df$py[(i-1)]*myz + df$pz[(i-1)]*mxz
}
```
...
ggplot( df ) + 
  geom_line( aes(t,px ), color="green" ) + 
  geom_line( aes(t,py ), color="blue" ) + 
  geom_line( aes(t,pz ), color="red" ) + 
  theme_minimal()
